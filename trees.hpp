...
namespace ft
{
	...
	struct	node_ds
	{
		...
		node_ds(value_type val, node_ds* parent) : data(val), p(parent)
		{
			left = NULL;
			right = NULL;
			isred = true;
	...
	> class trees
	{
		public:
			...
			typedef ft::node_ds<value_type>							node;
			typedef ft::tree_iterator<node, const value_type>				iterator;
		...
		public:
			...
			std::size_t erase (const value_type& k)
			{
				current = &root;
				if (_size == 1 && (*current)->data == k)
				{
					clear_node(root);
					root = NULL;
					_size = 0;
				...
				{
					if ((*current)->left && (*current)->data == k)
					{
			...
			ft::pair<iterator,bool> insert (const value_type& val)
			{
				...
				{
					p = *current;
					if ((*current)->left && (*current)->data == val)
			...
			std::size_t count( const value_type& key ) const
			{
				node*	tmp = root;
				while (tmp && tmp->left != NULL)
				{
					if (tmp->left && tmp->data == key)
						return(1);
					else if (_comp(key, tmp->data))
						tmp = tmp->left;
					else
						tmp = tmp->right;
				}
